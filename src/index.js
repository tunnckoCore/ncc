...
module.exports = (
  ...
  if (entry instanceof Array) {
    ...
    Object.keys(entry).forEach((item) => {
      ...
      else processedEntry[item] = pathResolve(entry[item]);
    });
  ...
  if (!quiet) {
    ...
    console.log(
      `ncc: Compiling file${
        Object.keys(processedEntry).length > 1 ? 's' : ''
      } ${Object.values(processedEntry).join(', ')}`,
    );
  }
  ...
  resolvePlugins.push({
    apply(resolver) {
      ...
      resolver.resolve = function(
        ...
        resolve.call(resolver, context, path, request, resolveContext, function(
          err,
          result,
        ) {
          if (!err) return callback(null, result);
          if (!err.missing || !err.missing.length) return callback(err);
  ...
  if (externals instanceof Array)
    ...
    Object.keys(externals).forEach((external) =>
      externalMap.set(external, externals[external]),
    );
  ...
  const compiler = webpack({
    ...
    optimization: {
      ...
      mangleExports: false,
      splitChunks:
        typeof entry === 'object' && Object.keys(entry).length > 1
          ? {
              chunks: 'all',
            }
          : false,
    ...
    externals: async ({ context, request }, callback) => {
      if (externalMap.has(request))
        return callback(null, `commonjs ${externalMap.get(request)}`);
      return callback();
    },
    module: {
      rules: [
    ...
    plugins: [
      {
        apply(compiler) {
          ...
          compiler.hooks.compilation.tap(
            'relocate-loader',
            relocateLoader.initAssetPermissionsCache,
          );
  ...
  if (!watch) {
    ...
    watcher = compiler.watch({}, (err, stats) => {
      if (err) return watchHandler({ err });
      if (stats.hasErrors()) return watchHandler({ err: stats.toString() });
  ...
  function finalizeHandler(stats) {
    ...
    for (const [key, value] of Object.entries(relocateLoader.getSymlinks())) {
      ...
      if (resolved in files && key in files === false) symlinks[key] = value;
    }
    ...
    function finalizeOutput(filename) {
      ...
      if (map) {
        ...
        map.sources = map.sources.map((source) => {
          ...
          while (source.startsWith('webpack:///')) source = source.substr(11);
          if (source.startsWith('./')) source = source.substr(2);
      ...
      if (sourceMap && sourceMapRegister) {
        ...
        files['sourcemap-register.js'] = {
          source: fs.readFileSync(
            __dirname + '/sourcemap-register.js.cache.js',
          ),
          permissions: defaultPermissions,
        };
      }
...
function getFlatFiles(mfsData, output, getAssetPermissions, curBase = '') {
  for (const path of Object.keys(mfsData)) {
    ...
    // directory
    if (item[''] === true)
      getFlatFiles(item, output, getAssetPermissions, curPath);
    // file
    else if (!curPath.endsWith('/')) {
      output[curPath.substr(1)] = {
        source: mfsData[path],
        permissions:
          getAssetPermissions(curPath.substr(1)) || defaultPermissions,
      };
    }
  }
}
