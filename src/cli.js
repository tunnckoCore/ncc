...
function renderSummary(files, outDir, buildTime) {
  ...
  const orderedAssets = Object.keys(files)
    .filter((file) => typeof files[file] === 'object')
    .sort((a, b) => {
      if (
...
async function runCmd(argv, stdout, stderr) {
  ...
  switch (args._[0]) {
    ...
    case 'build':
      const buildFiles =
        runArgs || args._.length === 2 ? resolve(args._[1]) : args._.slice(1);

      let startTime = Date.now();
      let ps;
      const ncc = require('./index.js')(buildFiles, {
        debugLog: args['--debug'],
        minify: args['--minify'],
